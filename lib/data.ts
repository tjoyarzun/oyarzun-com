 ...
 */
/**
 * The 2026 targets. The TARGET only — where we are against it is counted from
 * the arrays above, in lib/thrasher/issue.ts.
 *
 * These entries used to carry hand-kept `current` and `pct` values too. They
 * were read by nothing and had already drifted — 0 adventures against four
 * logged, 3 posts against two files — so anyone editing content would have
 * found two numbers for the same thing and corrected the wrong one.
 */
export const goals = [
